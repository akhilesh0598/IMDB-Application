...
export default {
  methods: {
    ...
    handleEdit(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          updateMovie(this.movieId, this.movie).then((response) => {
  ...
  mounted() {
    getMovie(this.movieId).then((response) => {
      ...
      this.movie.actorsIds = movie.actors.map(function (x) {
        return x.id;
      });
      this.movie.genresIds = movie.genres.map(function (x) {
        return x.id;
      });
      this.movie.imageUrl = movie.imageUrl;
    });
